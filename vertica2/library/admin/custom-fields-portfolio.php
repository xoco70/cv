...
array(
	...
	"_portfolio_imgpopup_type" => array(
	"name" => "_portfolio_imgpopup_type",
	"std" => "",
	"title" => "Portfolio image popup",
	"description" => "Please select portfolio image on click type.",
	"type" => "select",
	"options"     => array(
			array(
			  'value'       => 'default',
	...
	"_portfolio_v" => array(
		...
		"description" => "Put Here portfolio Video Link",
		"type" => "text",
		"location" => "portfolio"
	),
	"_portfolio_cl" => array(
		"name" => "_portfolio_cl",
		"std" => "",
		"title" => "Portfolio Custom link",
		"description" => "Put Here portfolio Custom Link with this format http://www.domain.com",
		"type" => "text",
		"location" => "portfolio"
	),
);
...
function new_meta_boxes1_portfolio() {
	new_meta_boxes1('portfolio');
}

function new_meta_boxes1( $type ) {
	global $post, $new_meta_boxes1;
	
	// Use nonce for verification
    echo '<input type="hidden" name="bc_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
function create_meta_box1() {
	...
	if ( function_exists('add_meta_box') ) {
		add_meta_box( 'new_meta_boxes1_portfolio', 'Vertica'  . ' portfolio Settings', 'new_meta_boxes1_portfolio', 'portfolio', 'normal', 'high' );
	}
}

function save_postdata1( $post_id ) {
	
	if ( !wp_verify_nonce(isset($_POST['bc_meta_box_nonce']) ? $_POST['bc_meta_box_nonce'] : '', basename(__FILE__)) ) {
